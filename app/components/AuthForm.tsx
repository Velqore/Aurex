  ...
  const handlePasswordLogin = async () => {
    ...
    try {
      const result = await otpService.authenticateWithPassword(
        ...
        console.log('✅ Password login successful, setting auth state:', result.user);
  ...
  const handleOtpLogin = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (!result.otpSent) {
          console.warn("OTP was not actually sent - user may not exist");
        }
      } else {
        setError(result.message || "OTP could not be sent. Please try again.");
      }
    } catch (error) {
